...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      .ts-row { display: flex; justify-content: space-between; align-items: center; gap: 10px; }
    ...
    function apply(ctx) {
      ...
      function TokenStatsView(props) {
        ...
        const row = (label, value) =>
          React.createElement('div', { className: 'ts-row', key: label },
            React.createElement('span', { className: 'ts-label' }, label),
            React.createElement('span', { className: 'ts-val' }, value));

        const summary = breakdownSummary(stats.breakdown);
        ...
        return React.createElement('div', { className: 'ts-wrap' }, [
          row('Tokens', usage ? compactTokens(totalTokens(usage)) : '—'),
          row('缓存命中', hit !== null ? hit + '%' : '—'),
          React.createElement('div', { className: 'ts-row', key: '费用', title: summary || undefined },
            React.createElement('span', { className: 'ts-label' }, '费用'),
            React.createElement('span', { className: 'ts-val' }, usage ? '¥' + fmt(cost, 4) : '—')),
          extra && extra.tokens > 0 ? row('系统调用', compactTokens(extra.tokens) + ' · ¥' + fmt(extra.cost, 4)) : null,
          React.createElement('div', { className: 'ts-row', key: '余额', title: balanceTip || undefined },
          ...
          React.createElement('div', { className: 'ts-row', key: '上下文' },
